...
racial/ethnic group. Due to the data being publically available, the
present study did not require IRB approval. Use of the data was
consistent with NHANES protocols for data visualizations. Data from
4,843 participants was gathered, with the average age of participants
being (SD = ) years old. The majority of participants were women
(\texttt{r\ td1\_female\$sex}), Hispanic (), and made 100,000 or more
(). Table 1 presents the breakdown of the sample based on
race/ethnicity.
...
Analyses were conducted using R and several packages, such as: nhanesA,
the tidyverse, rio, and the here package. Data was downloaded from the
NHANES 2015-2016 website, using the nhanesA package from R (ref). For
replicability purposes, the data was downloaded and transformed to .csv
files as a precaution to potential changes made to the NHANES datasets.
Consistent with open science, the present study's script is available at
\href{https://github.com/schweercollins/Final2018}{Final 2018 github}.
The datasets of the study variables were then merged to create one
master dataset with all the key variables. Study variables were re-coded
to have meaningful names rather than the names provided by the NHANES
website. The data was then cleaned and transformed to create variables
that were useful in the visual analyses. For analyses that utilized
depression scores, a sum score was created to capture several components
of depression and the relationship that depression has with other key
variables. This study presents its findings through visuals. The first
visual displays the rates of depression across different racial/ethnic
groups and whether or not they have utilized mental health services. The
second plot shows age at which different racial/ethnic groups engaged in
opioid and stimulant drug use. To examine how many individuals reported
drug use ever, the frequency of drug use was gathered to show the number
of participants that had reported stimulant and opioid drug use based on
racial/ethnic group. Similarly, average depressions core was also
examined by different racial/ethnic groups comparing those that reported
opioid use and stimulant use. A table was created to examine overall
depression scores, opioid use, and stimulant use across all
racial/ethnic groups. Lastly, to examine the associations between age at
first drug use and depression scores, a scatterplot was created to see
any racial/ethnic differences for opioid users and stimulant users.
...
As with any other study, there were limitations to the present study.
First, the findings stated for this study are based on visual
interpretations and should not be interpreted as causal as no
inferential statistics were conducted. Secondly, while the data was
gathered from a national database, it is possible that the data does not
fully represent racial/ethnic groups throughout the U.S. Finally, with
the majority of the data gathered through self-report measures, there is
potential bias in the answers, particularly for drug use and depression
questions.

\subsection{Implications}\label{implications}

The implications for future research is to examine the associations
between race/ethnicity, depression, and drug use based on insurance and
use of mental health services through inferential statistical techniques
to note if depression and drug use are different across different
races/ethnicities. The clinical implications are that depression and
drug use still differ across race/ethnicity suggesting that there are
health disparities across the U.S. Knowing that there are differences in
depression rates for different races/ethnicities based on uses of mental
health services, it is crucial to find routes to getting mental health
help for racial/ethnic groups that are experiencing high rates of
depression.
