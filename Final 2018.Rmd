...
```{r select_rename}
td1 <- td %>% 
  select(-2:-15, -HIQ011, -HUQ090, -DUQ430) %>% 
  rename(age_marijuana = DUQ210, age_cocaine = DUQ260, age_heroin = DUQ300, age_meth = DUQ340)

#reorder
td1 <- td1 %>% 
  select(1:6, age_stim, sum_depression, rehab, mental_health, insurance, 7:10)
```
...
#Create boxplot of total depression scores by gender
  #if this doesnt work anymore may just need to change gender to sex
