...
header-includes:
  - \raggedbottom # makes it look better
  - \setlength{\parskip}{0pt} # makes it look better
  - \usepackage{xcolor} # Just so I can make some things red
...
# Not sure if it makes a difference in this case but in the `merge` functions 
# above you're actually doing an inner join, which means any id that is not 
# represented in BOTH datasets will be dropped. This may or may not be what you
# wanted/intended.
...
# I changed it to a tibble because I basically can't stand working with 
# regular data frames interactively anymore.

#computing depression total score
td0 <- td0 %>%
  mutate(sum_depression = DPQ010 + DPQ020 + DPQ030 + DPQ040 + DPQ050 + DPQ060 + DPQ070 + DPQ080 + DPQ090)

# Does DPQ100 represent something different? 

# Another more difficult way you could do this would be:
#
# td0 %>%
#   gather(var, depression, starts_with("DPQ0")) %>%
#   group_by(V1) %>%
#   summarize(sum_depression = sum(depression, na.rm = TRUE)) %>%
#   right_join(td0) %>%
#   select(-starts_with("DPQ0"))
#  
# Although way more complicated in this example, this is sort of the way I'd 
# like your thinking to start to shift toward. The reason is because this is
# essentially the principles that tidy data are built upon, and particularly 
# if you have multiple variables you need to gather up like this, it can lead 
# to large gains in efficiency (even if not so much here).
...
# Is the second line here any different than
#   
# td0 %>% 
#   mutate(DUQ340 = ifelse(DUQ340 == 999, NA, DUQ340))
...
# We didn't talk about this in class at all, and I even nudged you toward 
# `case_when` (and I think it's an excellent approach in this case), but an
# alternative is to just create another dataframe and then merge it. e.g., 
#
# eth <- data_frame(ethnicity = c("Hispanic", 
#                                 "Hispanic",
#                                 "White",
#                                 "Black",
#                                 "Asian",
...
# it appears to me that you're defining these things as a factor twice... any
# reason?
...
```
## Measures
<!-- In the below, you should mention how “Refuse to answer” and “Don’t know” are scored. I'm assuming missing, but that's very different than zero, which on a raw-score basis would be treated the same. -->

<!-- I'm also assuming you wanted these to show up as separate paragraphs, but they're showing up as a single paragraph currently. I changed it so they'd be separate. -->
...
To explore health disparities among racial/ethnic groups, we employed a variety of data visualizations with our variables of interest. First, we plotted the frequency of substance use across ethnicities according to opioid and stimulant use. Then we plotted distributions of age of first substance use for each ethnic group. To assess depression, we first plotted average depression scores by ethnic group and explored the association of age of first substance use and depression scores across ethnic groups by fitting a linear regression in our plot. Lastly, we explored differences in depression scores among the racial categories according to whether or not individuals had insurance coverage or has reported using mental health services. 
...
mix_st <- inline %>% 
  ...
  select(stimulant_use)
```
<!-- I've changed this above but I'll let you decide what to do with it from 
here on out (i.e., I won't change it), but I generally like to either use 
"individuals who identified as..." or "individuals coded as..." when refering 
to race/ethnicities, depending on how the data were collected (i.e., if they 
did it versus someone else, in the case of little kids). There's some research
that says race/ethnicity is rather fluid in personal identity, particularly 
for people from mixed race backgrounds. -->
...
In our sample, 215 participants reported either opioid or stimulant use. To explore racial/ethnic disparities in reported use, we plotted the number of individuals who reported using opioids and/or stimulants based on their racial category. As shown in Figure 1, more Hispanic participants reported opioid and stimulant use, whereas Asian participants had the lowest reported frequency of use for both substances. To further explore disparities in substance use, we visualized the age of first use by ethnicity. Figure 2 shows that across both substance types Hispanic individuals have the most variability in their age of first use. Additionally, Asian participants have much more variability in their age of first opioid use compared to stimulants. This observation is slightly observed among Black and White individuals, but was more prominent among Asian individuals. Overall, the majority of participants reported their age of first use to be between ages 15-25 for both substance types. However, the density was generally greater in this range for reported stimulant use than reported opioid use. 

<!-- Shouldn't ever need `echo = FALSE` in a papaja code chunk, because that's 
the default -->

```{r fig.cap = "Number of Individuals Reporting Drug Use by Ethnicity"}
pd_st1 <- td1 %>% 
...
ggplot(pd_st1, aes(x = ethnicity, y = n)) +
  ...
  facet_wrap(~ drug) +
  labs(subtitle = "Based on two drug types (opioids and stimulants)",
       caption = "Source: NHANES",
       x = "Ethnicity",
       y = "Frequency") +
  theme(legend.position = "none")

# Might consider putting the in with the caption
```

```{r fig.height = 4, fig.cap = "Age of First Use by Ethnicity"}
pd_st <- td1 %>% 
           select(id, age_stim, age_heroin, sum_depression, rehab, ethnicity) %>%
           rename(age_Stimulants = age_stim,
                  age_Opioids = age_heroin) %>% 
...
Since depression and substance are strongly linked in the literature, we also produced a plot to explore racial disparities in average depression scores by substance type. As can be seen in Figure 3, Asian individuals that reported stimulant use had the highest reported average depression score compared to all other groups. For opioids, White participants had the highest reported depression score. To further explore the relation between age of first substance use and depression, we graphed age of first use by the total depression score and faceted by substance type. Then, we overlaid a best fit linear regression onto the graph according to racial groups (see Figure 4). Interestingly, the plot suggests that there is a positive association between age of first opioid use and depression for Black individuals and a negative association for all other racial groups. Additionally, we observed a positive relationship between age of first stimulant use and depression for all racial groups, however the slope of the relationship was more robust for Asian individuals. 

<!-- To me, Figure 4 is a little bit odd... Basically, you're saying, for 
opioid use, that generally, people coded Black were more depressed the later 
in life the *first* used opioids. That... is hard to interpret and I'm not 
sure what it really means. But this isn't really my field so maybe I just 
don't understand something here. 
 -->
```{r fig.cap = "Average Depression Score by Ethnicity"}
pd_st3 <- td1 %>% 
           select(id, age_stim, age_heroin, sum_depression, rehab, ethnicity) %>%
           rename(age_Stimulants = age_stim,
                  age_Opioids = age_heroin) %>% 
...
```{r fig.height = 4, fig.cap = "Association between Age at First Use and Depression"}
pd_st2 <- td1 %>% 
           select(id, age_stim, age_heroin, sum_depression, rehab, ethnicity) %>%
           rename(age_Stimulants = age_stim,
                  age_Opioids = age_heroin) %>% 
...
ggplot(pd_st2, aes(x = age_use, y = sum_depression, color = ethnicity)) + # try switching color and facet
  ...
  facet_wrap(~ drug) +
  labs(subtitle = "Colored to show differences among drug types",
       caption = "Source: NHANES",
       x = "Age at first use ",
...
```{r warning = FALSE, fig.height = 4, fig.cap = "Total Depression Score by Ethnicity and Usage of Mental Health Services"}
...
td1 %>%
  ...
  geom_col(aes(fill = mental_health),
           position = "dodge") +
   labs(subtitle = "Faceted by Insurance Coverage",
         caption = "Source: NHANES",
         x = "Ethnicity",
         y = "Total Depression Score",
...
<!-- Added a line break here to make it two paragraphs again, which I'm 
  assuming you wanted -->
...
<!-- In the above, this would have been a good spot for inline code -->
